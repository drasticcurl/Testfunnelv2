...
import { calcularTipo, calcularSeveridad } from '@/lib/tipos-hinchazon';
import { getSupabase } from '@/lib/supabase';
import { sendDiagnosticoEmail } from '@/lib/email/resend';
import { TIPOS_HINCHAZON } from '@/lib/parse-resultados';
import type { QuizAnswers } from '@/lib/quiz-types';

export const runtime = 'nodejs';

type SubmitBody = QuizAnswers & {
  email?: string;
  nombre?: string;
  fbc?: string;
  fbp?: string;
...
export async function POST(req: NextRequest) {
  ...
  const tipo = calcularTipo(body);
  const severidad = calcularSeveridad(body);
