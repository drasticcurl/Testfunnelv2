    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-[16px] p-4 border border-gray-100 shadow-sm">
        <div className="flex items-center justify-between mb-2">
          ...
          <span className="text-xs font-semibold text-sage">{progressPercent}%</span>
        </div>
        <div className="h-2 bg-gray-100 rounded-full overflow-hidden">
          <div
            className="h-full bg-gradient-to-r from-sage to-coral rounded-full transition-all duration-500 ease-out"
      ...
      {/* Week sections */}
      {[
        { label: 'Semana 1 — Limpieza', days: PLAN_DATA.slice(0, 7) },
        { label: 'Semana 2 — Reincorporación', days: PLAN_DATA.slice(7, 14) },
        { label: 'Semana 3 — Optimización', days: PLAN_DATA.slice(14, 21) },
        { label: 'Semana 4 — Mantenimiento', days: PLAN_DATA.slice(21, 30) },
      ].map((week, weekIdx) => {
      ...
      {!hasUpsell && (
        <div className="bg-coral-soft border border-coral rounded-[16px] p-4 text-center">
          <p className="font-serif font-semibold text-charcoal">
            ¿Querés los 30 días completos?
          </p>
          <p className="text-sm text-gray-600 mt-1 mb-3">
            Desbloqueá las semanas 2, 3 y 4 con el Programa 30 Días
          </p>
          <button className="bg-coral text-white font-semibold text-sm px-6 py-2.5 rounded-full hover:opacity-90 transition-opacity">
