/**
 * /upsell2-latam — SEGUNDO upsell del embudo PARALELO LATAM (Hotmart, USD).
 *
 * Text Sales Letter (TSL, sin video) del "Acceso VIP de por vida" (pago único,
 * US$27, ancla US$97). El comprador llega acá DESPUÉS de aprobar el pago del
 * upsell 1 (Programa de 30 Días, US$13.90). La transición upsell 1 → upsell 2
 * se configura en el panel de Hotmart (paso operativo manual), no en la app.
 *
 * Flujo:
 *   /upsell2-latam → CTA "SÍ"     → checkout Hotmart (LATAM_UPSELL2_CHECKOUT_URL)
 *   /upsell2-latam → "no gracias" → /downsell2-latam (segunda chance)
 *
 ...
 * Purchase del VIP lo dispara el webhook server-side de Hotmart.
 */

import { UpsellPageTracker } from '@/components/upsell/UpsellPageTracker';
import { Upsell2OfferLatam } from '@/components/upsell/Upsell2OfferLatam';
import {
  PRODUCT_SHORT_NAME_LATAM,
  PRICING_LATAM,
...
const FAQ = [
  {
    ...
    a: 'No. Es un pago ÚNICO. Lo abonás una sola vez y el acceso queda activo de por vida, con todas las actualizaciones futuras incluidas.',
  },
  {
    q: '¿En qué se diferencia de lo que ya compré?',
    a: 'Tu compra anterior es el programa principal. El Acceso VIP suma TODO lo premium: recetario ampliado, masterclasses, calculadora PRO, mini-guías y el protocolo anti-rebote, para siempre.',
  },
  {
    q: '¿Cómo accedo después de comprar?',
    a: 'Recibís un email con tu código VIP y el link a la sección VIP dentro de la app. Ingresás el código una vez y queda desbloqueada en tu dispositivo.',
  },
];
...
export default function Upsell2LatamPage() {
  return (
    <main className="min-h-screen bg-[#0F1116] text-white">
      ...
      <section className="bg-gradient-to-b from-[#0F1116] via-[#16181F] to-[#1F2433] pt-12 pb-10">
        <div className="max-w-content mx-auto px-4 text-center">
          ...
          <h1 className="mt-5 font-serif text-3xl md:text-5xl font-bold leading-tight text-white">
            Antes de entrar a tu app, querés escuchar esto:{' '}
      ...
      <section className="py-10 border-t border-white/10">
        <div className="max-w-content mx-auto px-4 text-center">
          ...
          <p className="mt-3 font-sans text-sm text-white/60 max-w-xl mx-auto leading-relaxed">
            ...
            una vez hoy ({PRICING_LATAM.upsell2.display}) y el acceso es para siempre.
          </p>
        </div>
      </section>
      ...
      <section className="border-t border-white/10">
        <Upsell2OfferLatam ctaLabel="👑 Sí, quiero mi Acceso VIP de por vida 👑" />
      </section>
